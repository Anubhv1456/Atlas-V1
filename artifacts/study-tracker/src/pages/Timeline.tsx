...
// ── Map a HistoryEntry → TimelineEvent ───────────────────────────────────────
  ...
  return (
    <div className="bg-card border rounded-xl p-3 flex items-center gap-3 shadow-sm">
      ...
      <div className="flex-1 min-w-0">
        ...
        {event.subjectName && (
          <p className="text-xs text-muted-foreground truncate">{event.subjectName}</p>
        )}
      </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-3">
        ...
        {events.length > 0 && (
          <span className="ml-auto text-xs font-semibold bg-muted text-muted-foreground px-2 py-0.5 rounded-full">
            {events.length}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {events.length} task{events.length !== 1 ? 's' : ''} completed
          </p>
        </div>
      </div>
...
export default function Timeline() {
  ...
  // ── Convert history → TimelineEvents for the visible month ───────────────
  const monthEvents: TimelineEvent[] = history
    .map(historyToEvent)
    .filter(e => e.date >= monthStart && e.date <= monthEnd);

  // Placeholder hooks for future Upcoming / Overdue events.
  // When the revision engine is ready, append events here.
  const upcomingEvents: TimelineEvent[] = []; // TODO: revision engine
  const overdueEvents:  TimelineEvent[] = []; // TODO: revision engine

  // Combine all for calendar dot rendering
  const allVisible = [
    ...monthEvents,
    ...upcomingEvents.filter(e => e.date >= monthStart && e.date <= monthEnd),
    ...overdueEvents.filter(e => e.date >= monthStart && e.date <= monthEnd),
  ];

  // ── Apply filter ──────────────────────────────────────────────────────────
  const filtered = (events: TimelineEvent[]) =>
  ...
  const activeDates = new Set(
    filtered(allVisible).map(e => format(e.date, 'yyyy-MM-dd')),
  );

  // ── Sections ──────────────────────────────────────────────────────────────
  const todayEvents   = filtered(monthEvents).filter(e => isToday(e.date));
  const filteredUpcoming = filtered(upcomingEvents.filter(e => e.date >= monthStart && e.date <= monthEnd));
  const filteredOverdue  = filtered(overdueEvents.filter(e => e.date >= monthStart && e.date <= monthEnd));

  // Past entries in the month, excluding today, grouped by day (most recent first)
  const pastEntries = filtered(monthEvents).filter(e => !isToday(e.date));
    ...
    <div className="min-h-[100dvh] bg-background px-4 pt-8 pb-24 max-w-3xl mx-auto">
      <header className="mb-8">
        ...
        <p className="text-sm text-muted-foreground">
          Your chronological view of knowledge.
        </p>
      </header>
      ...
      <div className="space-y-8">
        <Section
          ...
          events={todayEvents}
          emptyText="Nothing completed today yet."
        />
        ...
        {/* Empty state — nothing at all in this month */}
